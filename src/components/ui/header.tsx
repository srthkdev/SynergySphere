...
export function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (window.scrollY > 10) {
        navRef.current?.classList.add("bg-white", "border-gray-200", "shadow-sm");
        navRef.current?.classList.remove("bg-white/80", "border-gray-100");
      } else {
        navRef.current?.classList.add("bg-white/80", "border-gray-100");
        navRef.current?.classList.remove("bg-white", "border-gray-200", "shadow-sm");
      }
    };
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-4 py-2 bg-transparent">
      <div 
        ref={navRef}
        className={cn(
          "max-w-7xl mx-auto p-2 flex justify-between items-center backdrop-blur-sm border-gray-100 rounded-lg transition-all duration-200",
          "bg-white/80"
        )}
      ...
      >
        <div className="flex-shrink-0">
          <Link href="/" className="flex items-center space-x-2">
            <LogoIcon className="h-8 w-8" forceLightMode={true} />
          </Link>
        </div>
              ...
              <SheetContent className="px-3 pt-8">
                <div className="flex flex-col justify-between h-full">
                  <div className="flex flex-col gap-4">
                    <button 
                      onClick={() => scrollToSection('features')} 
                      className="text-base text-gray-600 hover:text-gray-900 text-left"
                    >
                      Features
                    </button>
                    <button 
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="outline-none">
                <div className="flex items-center space-x-1 cursor-pointer">
                  <Avatar className="h-8 w-8 ring-2 ring-blue-500/20 ring-offset-1 ring-offset-white">
              ...
              <DropdownMenuContent align="end" className="w-80 p-2">
                <div className="flex flex-col items-center p-4 space-y-3">
                  <Avatar className="h-16 w-16 ring-2 ring-blue-500/20 ring-offset-1 ring-offset-white shadow-md">
